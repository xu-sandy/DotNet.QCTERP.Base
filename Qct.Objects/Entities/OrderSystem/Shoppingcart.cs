...
namespace Qct.Objects.Entities
{
    ...
    public class Shoppingcart : IOrder
    {
        ...
        public void AddItem(ProductRecord product, bool isGift, decimal num)
        {
            if (num == 0m) throw new OrderException("购买数量不能为0！");

            var saleState = SaleState.Normal;
            var price = product.SysPrice;
            ...
            Items.Add(
                ...
                {
                    Id = Guid.NewGuid(),
                    Number = new ProductNumber(num, product.SubUnit, product.IsWeightProduct()),
                    Product = product,
                    SaleState = saleState,
                    ManualPrice = price,
